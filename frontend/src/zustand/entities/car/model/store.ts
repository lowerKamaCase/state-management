...
/**
 * The store itself never leaves this file — only `useCarsList` is exported,
 * so no other module can reach in and call `fetchCars` directly. A mutation
 * feature that needs a fresh list calls the `refetch()` this hook returns.
 */
const useCarsListStore = create<CarsListState>((set) => {
  return {
    cars: [],
    meta: null,
    isLoading: false,
